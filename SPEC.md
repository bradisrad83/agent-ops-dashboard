...
### Run
A *Run* is a top-level execution triggered by a user prompt (or a system action).
- A run contains many events.
- A run may have many agents participating.

### Agent
An *Agent* is a named worker (orchestrator, planner, builder, reviewer, etc.)
- Agents emit events.
- Agents may execute tools.
- Agents may produce artifacts.

### Event
An *Event* is an append-only record describing something that happened.
- The UI is driven entirely by an event stream.
- Events are immutable once written.

### Artifact
An *Artifact* is a produced/consumed resource (file, diff, link, JSON, etc.)
